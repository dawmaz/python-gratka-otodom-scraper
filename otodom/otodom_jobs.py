...
def full_scan(url):
    ...
    logger.info(f'Full scan started for otodom. Number of pages: {page_numbers}')
    ...
    logger.info(f'Full scan ended for otodom. Number of messages sent to queue: {length}')
...
def refresh_all():
    ...
    logger.info(f'Refresh all started for otodom offers visited before {one_day_ago}')
    with create_session() as session:
        ...
        logger.info(f'Refresh all ended for otodom. Number of messages sent to queue: {len(offers_addresses)}')
        return len(offers)
...
def add_download_photos_to_queue(params, offer_id, foreign_id):
    ...
    logger.info(f'Sent {len(concat_images_with_id)} otodom images of {offer_id} to process image queue')
